...
public class MainView extends VerticalLayout {
    ...
    private static ArrayList<HorizontalLayout> resultLayouts;

    // Text fields for input for player
    private static ArrayList<TextField> lines;
    ...
    public MainView(SecurityService securityService) {
        ...
        if(SecurityUtils.isUserLoggedIn()) {
            playerWelcome.setText("Welcome back, " + securityService.getAuthenticatedUser());
        }
            ...
            for (int i = 0; i < lines.size(); i++) {
                if (lines.get(i).isEnabled()) {
                    ...
                    if(lines.get(i).getValue().length() != 5) {
                        ...
                        // DOES NOT SUBMIT GUESS, WORD IS < 5 CHARS
                        ...
                        boolean isInputValid = WordleHandler.checkUserGuess(input.toString());

                        if(isInputValid) {
                            WordleHandler.submitWord(currLine, input.toString());
                        }
     ...
     */
    public static void setResult(int line, int col, int status, char input) {
